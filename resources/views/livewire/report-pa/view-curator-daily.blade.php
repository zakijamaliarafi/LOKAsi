...
use App\Mail\CuratorDailyPAReport;
...
new class extends Component {
    ...
    public function getInfo(){
        ...
        ->leftJoin('reports_pa', function ($join) use ($dateObj) {
            $join->on('users.id', '=', 'reports_pa.curator_id')
                 ->whereDate('reports_pa.curate_time', $dateObj);
        })
        ->select(
    ...
    public function send(){
        ...
        foreach ($poiData as $poi) {
            fputcsv($csvData2, [$poi->id, $poi->status, $poi->reject_reason, $poi->street_name, $poi->street_name_status, $poi->house_number, $poi->house_number_status, $poi->house_number_update, $poi->latitude, $poi->longitude, $poi->curate_time, $poi->curator_name]); 
        }
        rewind($csvData2);
        $csvOutput2 = stream_get_contents($csvData2);
        fclose($csvData2);

        Mail::to('lokasi.terra@gmail.com')->send(new CuratorDailyPAReport($csvOutput, $csvOutput2));
    }
